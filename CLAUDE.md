...
- Axios (**`lib/axios.ts`**): **`sessionEndReason`** decides when a response ends the session. Any **`401`** (`UNAUTHORIZED`, `TOKEN_REVOKED`) and a **`403`** with `error.code` **`ACCOUNT_DISABLED`** (the admin was blocked or deleted) clear the session and hard-redirect with `location.replace` to **`/login`**, or **`/login?reason=account_disabled`** for a disabled account (the login page shows **`common:auth.account_disabled`**). The interceptor then returns a promise that never settles, so no caller toasts or retries with the old token. Do not call **`POST /admin/auth/logout`** first; it gets the same `403`. A **`403 FORBIDDEN`** is a missing permission: it toasts **`common:errors.permission_denied`** and keeps the session. None of this applies to **`/admin/auth/login`**, **`/admin/auth/forgot-password`**, **`/admin/auth/forgot-password/resend`**, or **`/admin/auth/reset-password`**, which report errors on the form (a blocked admin signing in sees the localized `ACCOUNT_DISABLED` copy). Admin routes always answer in English, so translate by `error.code`. The bearer comes from persisted Zustand storage in **`localStorage`**; helpers **`extractApiErrorMessage`** + **`rejectionMessage`** prefer nested **`data.message`** when present.
...
- Create/edit: all four fields are required (`fullName` 2 to 100 characters, `email`, `phone`, `roleId`); there is no password field. **`POST /admin/admins`** returns **`201`** with `data.admin` and `data.welcomeEmailQueued`. `true` toasts the success copy with the email; `false` means the admin exists but the welcome email was not queued, so a warning tells the operator to have them use **Forgot password?**. Never retry the create. There is no resend-welcome endpoint.
- Field errors: **`admins.queries.ts`** maps `error.code` to a form field. `ADMIN_EMAIL_ALREADY_EXISTS` goes to **email** and `ADMIN_PHONE_ALREADY_EXISTS` to **phone** (create and edit, both `409`). `RESOURCE_NOT_FOUND` goes to **role** (create only; it also clears the selection and refetches roles). These codes skip the toast via **`silentCodes`** on **`useResourceMutation`**; the page sets them with `setError(..., { shouldFocus: true })` and keeps the dialog open.
...
- **Responsive breakpoint is `lg` (1024px).** At `lg`+ the sidebar is the persistent rail (collapse preference applies); below `lg` it becomes an off-canvas **drawer**. The switch is driven by **`useIsDesktop()`** (`lib/use-media-query.ts`, matches `(min-width: 1024px)`), not CSS alone, because the rail width / content offset are inline styles. **Desktop behavior is unchanged** — every mobile rule is gated behind `lg:` or `isDesktop`. The drawer state is **`useUIStore.mobileNavOpen`** (transient, `partialize`d out of persistence; only `sidebarCollapsed` persists). `Shell` sets `marginInlineStart: 0` below `lg` and renders the scrim `<button>` (`z-[var(--z-overlay)]`, `lg:hidden`) that calls `closeMobileNav`. `Sidebar` translates off-canvas via inline `transform` (RTL-aware: `translateX(±100%)` from `i18n.dir()`), is `inert` when off-canvas, forces full width + expanded labels in the drawer (`collapsed = isDesktop ? pref : false`), hides the collapse toggle (`hidden lg:block`), and closes itself on every route change. `Topbar` renders a hamburger (`lg:hidden`) bound to `toggleMobileNav` and uses `shell:sidebar.open_menu`; the brand block is intrinsic-width below `lg` and the breadcrumb is `hidden sm:block md:hidden xl:block` (it shows from 640 to 767px and from 1280px up; between those the search pill and the rail leave it too little room and it would collapse to single letters). When adding chrome, keep the same gating so the desktop layout never regresses.
