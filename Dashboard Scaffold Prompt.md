...
- Route context carries `requiredPermissions: string[]`; layout renders a 403 inline panel if missing
...
Seed namespaces now, populated only for what the shell renders:
...
Every feature owns its own namespace later. **No hardcoded strings in components, ever.**
