...
export function UsersListPage() {
  ...
  const [activateTarget, setActivateTarget] = useState<AdminUserListItem | null>(null)
  ...
  /* ---------- row actions ---------- */
  function getRowActions(row: AdminUserListItem): RowActionItem<AdminUserListItem>[] {
    const items: RowActionItem<AdminUserListItem>[] = [
      {
        label: t('users:actions.view'),
        icon: Eye,
        onClick: (r) => navigate({ to: '/users/$userId', params: { userId: r.id } }),
      },
    ]

    if (canActivate && (row.status === 'banned' || row.status === 'suspended')) {
      items.push({
